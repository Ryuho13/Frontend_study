...
// HTML에서 해당 요소를 검색하여 찾은 첫 밴째 요소 하나만 반환
// 요소를 찾지 못하면 null을 반환
...
// 해당 요소에 지정한 이벤트가 발생하는지 듣고있다가
// 실제 이벤트가 발생하면 안결되 콜백 함수
// BoxE1.addEvabt(이벤트명, 함수)
boxEl.addEventListener('click', function (e) {
  console.log('Click!!')
  
  // (중요) this 와 event 객체
  console.log(this); // this: 이벤트 리스너에서의 this는 이벤트 바인딩된 요소를 가리킴(예: <div>)
  console.log(e.target); // e.target: 실제로 클릭이된 DOM 요소 (예: <div> or 자식 요소)
  ...
  //  활용 예: actine라는 클래스를 이용해 스타일을 지정하고 클릭한 특정 요소에 스타일을 적용할 때
